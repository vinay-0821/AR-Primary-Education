...
#pragma warning restore IDE0065
  ...
  public sealed class DetectionAnnotation : HierarchicalAnnotation
  {
    ...
    public Mediapipe.Unity.Object3DMap object3DMap;
    ...
    public void Draw(Detection target, float threshold = 0.0f)
    {
      ...
      {
        // Debug.Log("DetectionAnnotaiton Draw1");
    ...
    public void Draw(mptcc.Detection target, Vector2Int imageSize, float threshold = 0.0f)
    {
      if (ActivateFor(target))
      {
        // Debug.Log("DetectionAnnotation Draw2");
        ...
        _boundingBoxAnnotation.SetColor(GetColor(score, Mathf.Clamp(threshold, 0.0f, 1.0f)));
        _boundingBoxAnnotation.Draw(rectVertices);
        // Debug.Log($"Image size = {imageSize}, rotationAngle = {rotationAngle}, isMirrored = {isMirrored}");
        // Debug.Log($"{rectVertices[0]},{rectVertices[1]},{rectVertices[2]},{rectVertices[3]} ** {target.boundingBox}");
        ...
        {
            if(child.name == "3DObject")
            {
              //  Debug.Log("Found 3Dobject");
               object3D = child.gameObject;
               break;
            }
        }
        
        CreatePrefabByCategory(labelText,ref object3D);
       
        if(object3D != null)
        { 
          object3D.SetActive(true);
          object3D.transform.localPosition = new Vector3((rectVertices[1].x+rectVertices[2].x)/2,(rectVertices[1].y+rectVertices[0].y)/2 , 0);
        }
      }
    }
    ...
    private void CreatePrefabByCategory(string category, ref GameObject object3D)
    {
       if(_currentCategory != category)
       {
          if(category != null)
          {
              GameObject prefab = object3DMap.GetPrefabByCategory(category);
              if(prefab != null)
              {
                  Destroy(object3D);
                  object3D = Instantiate(prefab,this.transform);
                  object3D.name = "3DObject";
